...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const { save, open, isDirty } = usePersistence();
    ...
    <div className="flex h-screen overflow-hidden bg-background">
      <aside className="w-60 shrink-0 border-r border-border bg-sidebar flex flex-col">
        <div className="px-4 pt-4 pb-2 flex items-center gap-2">
          ...
          <span className={`inline-flex items-center gap-1 text-xs ${isDirty ? 'text-amber-600' : 'text-muted-foreground'}`}>
            <span className={`inline-block h-1.5 w-1.5 rounded-full ${isDirty ? 'bg-amber-500' : 'bg-emerald-500'}`} />
